...
STOP_WORDS = {
    ...
    "er", "es", "wir", "ich", "auf", "so", "eine", "auch", "als", "nach", "wie", "für", "man", "aber", "aus", "bei",
    "ein", "bis", "ist", "hat", "nur", "oder", "habe", "kann", "werden", "wenn", "doch", "mal", "app", "seit", "mehr",
    "immer", "wieder", "mich", "meine", "keine", "bin", "noch", "war", "jetzt", "alles", "gut", "sehr", "zum", "mir",
    "vor", "da", "am", "um", "denn", "einen", "einem", "einer", "muss", "wird", "sind", "wäre", "über", "wegen", "diese"
}
...
def get_semantic_topics(reviews, num_clusters=5):
    ...
    if len(text_reviews) < 5: return ["Zu wenige Daten"]
    ...
    prompt = f"""
    Du bist ein Produkt-Analyst. Erstelle für jedes dieser {len(topic_samples)} Reviews ein kurzes, prägnantes Schlagwort (max 2 Wörter).
...
def run_analysis_and_generate_html(full_history, new_only):
    ...
    if model and rich_reviews:
        ...
        2. Wähle 3 Top-Reviews und 3 Bottom-Reviews.
    ...
    # Python Smart Filter
    candidates_pos = [r for r in full_history if r['rating'] >= 4 and is_genuine_positive(r)]
    sorted_best = sorted(candidates_pos, key=lambda x: len(x['text']), reverse=True)
    for r in sorted_best[:3]: top_list.append(r)

    sorted_worst = sorted([r for r in full_history if r['rating'] <= 2], key=lambda x: len(x['text']), reverse=True)
    for r in sorted_worst[:3]: bot_list.append(r)

    # Wenn Python-Filter leer, nimm KI-Vorschlag
    ...
    <head>
        ...
        <style>
            :root {{ --bg: #f8fafc; --text: #1e293b; --card-bg: #ffffff; --border: #e2e8f0; --primary: #2563eb; --summary-bg: #eff6ff; --shadow: rgba(0,0,0,0.05); --ios: #000; --android: #3DDC84; --mark-bg: #fef08a; --mark-text: #854d0e; }}
            [data-theme="dark"] {{ --bg: #0f172a; --text: #e2e8f0; --card-bg: #1e293b; --border: #334155; --primary: #3b82f6; --ios: #fff; --summary-bg: #1e293b; --shadow: rgba(0,0,0,0.3); --mark-bg: #854d0e; --mark-text: #fef08a; }}
            ...
            <div class="row" style="display:flex; gap:20px; flex-wrap:wrap; margin-bottom:40px;">
                ...
                <div class="col" style="flex:1;">
                    ...
                    <div style="background:var(--card-bg); padding:15px; border-radius:12px; border:1px solid var(--border);">
                        {''.join([f'<span class="buzzword" style="font-size:{10 + (c/buzzwords[0][1])*10}px">{w} ({c})</span> ' for w, c in buzzwords])}
                    </div>
                </div>
            </div>
        ...
        <script>
            ...
            function filterData() {{
                ...
                filtered.slice(0, 50).forEach(r => {{
                    ...
                    // HIGHLIGHTING LOGIC (JS Concatenation Fix)
                    let displayText = r.text;
                    if (q.length >= 2) {{
                        const regex = new RegExp('(' + q + ')', 'gi'); // <--- HIER IST DER FIX
